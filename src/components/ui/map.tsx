...
type MapControlsProps = {
  ...
  /** Callback when geolocation fails (permission denied, unavailable, timeout) */
  onLocateError?: (error: { code: number; message: string }) => void;
};

/** Mensajes legibles para GeolocationPositionError (el objeto no serializa a JSON). */
export function describeGeolocationError(code: number): string {
  switch (code) {
    ...
    case 3:
      return "La búsqueda de ubicación tardó demasiado. Intenta de nuevo.";
    default:
...
function MapControls({
  ...
  const handleLocate = useCallback(() => {
    if (!("geolocation" in navigator)) {
      onLocateError?.({ code: 0, message: "Tu navegador no soporta geolocalización." });
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        // GeolocationPositionError no serializa a JSON ({}) — loggear code + message
        const message = describeGeolocationError(error.code);
        console.warn("Geolocalización falló:", { code: error.code, message, detail: error.message });
        onLocateError?.({ code: error.code, message });
        setWaitingForLocation(false);
      },
